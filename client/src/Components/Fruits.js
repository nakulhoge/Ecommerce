...
const Fruit = ({ searchQuery }) => {
  const { fruit } = UseAuth();
  const dispatch = useDispatch();

  // Check if fruit is an array before calling map function
  if (!Array.isArray(fruit)) {
    console.error("Fruit data is not an array:", fruit);
    return null; // Render nothing if fruit is not an array
  }

  const filterProduct = fruit.filter((fruit) =>
          ...
          <Col key={index} xs={6} sm={6} md={4} lg={3} xl={3} className="mb-4">
            <Card className="shadow h-100" style={{ height: "100%" }}>
              ...
              <Card.Body className="d-flex flex-column p-2 p-sm-3">
                ...
                <Card.Text className="text-muted mb-3 d-none d-sm-block">
                  {product.description}
                </Card.Text>{" "}
                {/* Hide description on mobile */}
                <Card.Text
                  className="text-muted mb-3 d-block d-sm-none"
